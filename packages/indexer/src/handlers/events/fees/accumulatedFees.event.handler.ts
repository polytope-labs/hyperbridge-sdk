...
export const handleAccumulateFeesEvent = wrap(async (event: SubstrateEvent): Promise<void> => {
	try {
		...
		logger.info(`accumulated fees event gotten for relayer ${relayerBytes} on chain ${stateMachine} with amount ${rawAmountCodec}`);
		...
		logger.info(`accumulated fees event gotten for relayer ${relayerBytes} on chain ${stateMachineId} with amount ${rawAmountCodec}`);
		...
		await RelayerService.updateFeesEarnedViaAccumulation(relayerAddress, normalizedAmount, stateMachineId, timestamp)
