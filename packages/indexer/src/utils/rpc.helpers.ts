...
/**
 * getTokenDecimals is a function that retrieves the decimals of a token given its symbol and chain.
...
export async function getTokenDecimals(symbol: string, chain?: string, contractAddress?: Hex): Promise<number> {
	...
	return getSubstrateTokenDecimals(symbol)
}

export async function getEvmTokenDecimals(chain: string, contractAddress: Hex): Promise<number> {
	const rpcUrl = replaceWebsocketWithHttp(ENV_CONFIG[chain] || "")
	if (!rpcUrl) {
		throw new Error(`No RPC URL found for chain: ${chain}`)
	}
	...
	try {
		...
		const { result } = (await response.json()) as { id: number; jsonrpc: number; result: Hex }
		return Number(result)
	} catch (err) {
		// @ts-ignore
		throw new Error(`Error fetching decimals for ${contractAddress} on ${chain}: ${err.message}`)
	}
}

export async function getSubstrateTokenDecimals(_symbol: string): Promise<number> {
	try {
		const symbolHash = keccak256(stringToBytes(_symbol))

		if (api.query.tokenGateway) {
			const asset = await api.query.tokenGateway?.localAssets<StorageData>(symbolHash)

			if (api.query.assetRegistry) {
				const response = await api.query.assetRegistry?.currencyMetadatas<StorageData>(asset.toHex())
		...
		const { tokenDecimals, tokenSymbol } = properties.toHuman() as {
			tokenDecimals: string[]
			tokenSymbol: string[]
		}

		const index = safeArray(tokenSymbol).findIndex((s) => s.toLowerCase() === _symbol.toLowerCase())
		if (index !== -1) {
			return Number(tokenDecimals[index])
		}

		throw new Error(`Unexpected response: No token decimals found for symbol ${_symbol}`)
	} catch (err) {
		throw new Error(`RPC error: ${(err as Error).message}`)
	}
}
