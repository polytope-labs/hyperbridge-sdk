...
export class IntentGatewayV2 {
	 ...
	 */
	async isOrderFilledV2(order: OrderV2): Promise<boolean> {
	 ...
	 */
	async isOrderRefundedV2(order: OrderV2): Promise<boolean> {
		const commitment = (order.id ?? orderV2Commitment(order)) as HexString
		const sourceStateMachineId = isHex(order.source)
			? hexToString(order.source as HexString)
			: order.source

		const intentGatewayV2Address = this.source.configService.getIntentGatewayV2Address(sourceStateMachineId)

		let fromBlock: bigint | undefined
		if (order.transactionHash) {
			const receipt = await this.source.client.getTransactionReceipt({ hash: order.transactionHash })
			fromBlock = receipt.blockNumber
		}

		const logs = await this.source.client.getLogs({
			address: intentGatewayV2Address,
	...
	async estimateFillOrderV2(params: EstimateFillOrderV2Params): Promise<FillOrderEstimateV2> {
		...
		const destStateMachineId = isHex(order.destination)
			...
			: order.destination
		const intentGatewayV2Address = this.dest.configService.getIntentGatewayV2Address(destStateMachineId)
		console.log({ intentGatewayV2Address })
