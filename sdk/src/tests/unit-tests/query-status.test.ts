...
describe("queryStatus", () => {
	...
	it("returns correct status and metadata for valid hash", async () => {
		...
		const result = await client.queryRequest(VALID_HASH)
	...
	it("throws error for invalid hash format", async () => {
		...
		await expect(client.queryRequest(INVALID_HASH)).rejects.toThrow("Invalid hash format")
	})
	...
	it("throws error when request not found", async () => {
		...
		await expect(client.queryRequest("0x0000000000000000")).rejects.toThrow("No request found")
	})
	...
	it("handles timed out requests", async () => {
		...
		const result = await client.queryRequest(TIMED_OUT_HASH)
	...
	it("handles malformed metadata response", async () => {
		...
		await expect(client.queryRequest(VALID_HASH)).rejects.toThrow()
	})
	...
	it("handles GraphQL endpoint connection error", async () => {
		...
		await expect(client.queryRequest(VALID_HASH)).rejects.toThrow("Failed to fetch")
	})
})
