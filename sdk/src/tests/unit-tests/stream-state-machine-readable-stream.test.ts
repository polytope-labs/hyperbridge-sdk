...
describe("createStateMachineUpdateStream", () => {
	...
	it("emits updates with correct data", async () => {
		const stream = client.createStateMachineUpdateStream("eth-sepolia", 100, "ethereum")
	...
	it("handles network errors with retry", async () => {
		jest.spyOn(client["client"], "request")
	...
	it("filters updates below starting height", async () => {
		...
		const stream = client.createStateMachineUpdateStream("eth-sepolia", startHeight, "ethereum")
		const reader = stream.getReader()
